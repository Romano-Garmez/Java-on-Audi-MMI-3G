 ...
 * with a particular LocationProvider. Applications implement this interface and
 * register it with a LocationProvider to obtain regular position updates.
 * 
 * When the listener is registered with a LocationProvider with some update
 * period, the implementation shall attempt to provide updates at the defined
 * interval. If it isn't possible to determine the location, e.g. because of the
 * LocationProvider being TEMPORARILY_UNAVAILABLE or OUT_OF_SERVICE or because
 * the update period is too frequent for the location method to provide updates,
 * the implementation can send an update to the listener that contains an
 * 'invalid' Location instance.
 * 
...
public interface LocationListener {
	...
	/**
	 * Called by the LocationProvider to which this listener is registered. This
	 * method will be called periodically according to the interval defined when
	 * registering the listener to provide updates of the current location.
	 * 
	 * @param provider
	 ...
	 * LocationProvider. If the application wishes to stop receiving location
