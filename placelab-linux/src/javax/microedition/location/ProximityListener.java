...
/**
 * This interface represents a listener to events associated with detecting
 * proximity to some registered coordinates. Applications implement this
 * interface and register it with a static method in LocationProvider to obtain
 * notfications when proximity to registered coordinates is detected.
 * 
 * This listener is called when the terminal enters the proximity of the
 * registered coordinates. The proximity is defined as the proximity radius
 * around the coordinates combined with the horizontal accuracy of the current
 * sampled location.
 * 
 * The listener is called only once when the terminal enters the proximity of
 * the registered coordinates. The registration with these coordinates is
 * cancelled when the listener is called. If the application wants to be
 * notified again about these coordinates, it must re-register the coordinates
 * and the listener.
 * 
 */
...
public interface ProximityListener {
	 ...
	 * 
	 * @param coordinates
	 *            the registered coordinates to which proximity has been
	 *            detected
	 * @param location
	 ...
	 *            a boolean indicating the new state of the proximity
