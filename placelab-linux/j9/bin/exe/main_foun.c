...
static void dumpHelpText( J9PortLibrary *portLib, int argc, char **argv, int *copyrightWritten)
{
	...
	j9tty_printf( PORTLIB, "  -jcl:foun         only the foun library is supported for j9foun\n");
...
UDATA VMCALL gpProtectedMain(void *arg)
{
...
#endif 
        /* j9foun does not allow use of any other lib than foun */
        if (!strncmp("-jcl:", argv[i], 5))
        {
            /* If the -jcl option is used it MUST be -jcl:foun */
            if (strncmp("-jcl:foun", argv[i], 8))
            {
                rc = 12;
                goto cleanup;
            }
	...
	if (!mainClass || (isStandaloneJar && jxeArg)) {
		/* no class found.  fail before we get any farther */
		dumpHelpText(j9portLibrary, argc, argv, &copyrightWritten);
		rc = 4;
		goto cleanup;
	}
	...
	if(NULL == (jclArg = strBufferCat(j9portLibrary, NULL, "-Xjcl:")))
		goto cleanup;
	if(NULL == (jclArg = strBufferCat(j9portLibrary, jclArg, J9_FOUNDATION_DLL_NAME)))
		goto cleanup;


    /* Only allow the CDC library to be used with the j9foun executable */
	if (!vmOptionsTableAddOption(&vmOptionsTable, strBufferData(jclArg), (void*) NULL))
		goto cleanup;
...
#if defined(JITNOCERT)
        case 11: /* case 11: -jit not certified on j9foun executable and only allow -jcl:foun */
            j9portLibrary->tty_printf(j9portLibrary, "VM startup error: JIT not available with j9foun\n");
            break;
#endif
    case 12: /* case 12: foun executable will only allow -jcl:foun option */
        j9portLibrary->tty_printf(j9portLibrary, "VM startup error: j9foun must use -jcl:foun\n");
        break;
