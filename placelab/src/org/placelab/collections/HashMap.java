  ...
  implements Map, Cloneable //, Serializable
{
   ...
   * Package visible for use by HashSet.
   */
  static final int DEFAULT_LOAD_FACTOR = 75; // percent
  ...
  public void clear()
  {
    if (size != 0)
      {
        modCount++;
        if(buckets != null) for(int i=0;i<buckets.length;i++) buckets[i] = null;
  ...
  public Object pclone()
  {
    ...
    // Clear the entry cache. AbstractMap.pclone() does the others.
    copy.entries = null;
    return copy;
  }
  ...
  /**
   * A simplified, more efficient internal implementation of putAll(). pclone() 
  ...
  private void rehash()
  {
    ...
    threshold = (newcapacity * loadFactor) / 100;
  ...
  private final class HashIterator implements Iterator
  {
    ...
    public boolean hasNext()
    {
      if (knownMod != modCount)
        throw new ConcurrentModificationException("HashMap.HashIterator.hasNext.4");
      return count > 0;
    }
    ...
    {
      if (knownMod != modCount)
        throw new ConcurrentModificationException("HashMap.HashIterator.next.5");
      if (count == 0)
    ...
    {
      if (knownMod != modCount)
        throw new ConcurrentModificationException("HashMap.HashIterator.remove.6");
      if (last == null)
