...
static void dumpHelpText( J9PortLibrary *portLib, int argc, char **argv, int *copyrightWritten)
{
	...
	j9tty_printf( PORTLIB, "  -jcl:cdc         only the cdc library is supported for j9cdc\n");
...
UDATA VMCALL gpProtectedMain(void *arg)
{
...
#endif 
        /* j9cdc does not allow use of any other lib than cdc */
        if (!strncmp("-jcl:", argv[i], 5))
        {
            /* If the -jcl option is used it MUST be -jcl:cdc */
            if (strncmp("-jcl:cdc", argv[i], 8))
            {
                rc = 12;
                goto cleanup;
            }
	...
	if(NULL == (jclArg = strBufferCat(j9portLibrary, NULL, "-Xjcl:")))
		goto cleanup;
	if(NULL == (jclArg = strBufferCat(j9portLibrary, jclArg, J9_CDC_DLL_NAME)))
		goto cleanup;


    /* Only allow the CDC library to be used with the j9cdc executable */
	if (!vmOptionsTableAddOption(&vmOptionsTable, strBufferData(jclArg), (void*) NULL))
		goto cleanup;
...
#if defined(JITNOCERT)
        case 11: /* case 11: -jit not certified on j9cdc executable and only allow -jcl:cdc */
            j9portLibrary->tty_printf(j9portLibrary, "VM startup error: JIT not available with j9cdc\n");
            break;
#endif
    case 12: /* case 12: cdc executable will only allow -jcl:cdc option */
        j9portLibrary->tty_printf(j9portLibrary, "VM startup error: j9cdc must use -jcl:cdc\n");
        break;
