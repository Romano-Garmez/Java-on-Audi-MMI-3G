...
static void dumpHelpText( J9PortLibrary *portLib, int argc, char **argv, int *copyrightWritten)
{
	...
	j9tty_printf( PORTLIB, "  -jcl:cldc         only the cldc library is supported for j9cldc\n");
...
UDATA VMCALL gpProtectedMain(void *arg)
{
...
#endif 
        /* j9cldc does not allow use of any other lib than cldc */
        if (!strncmp("-jcl:", argv[i], 5))
        {
            /* If the -jcl option is used it MUST be -jcl:cldc */
            if (strncmp("-jcl:cldc", argv[i], 8))
            {
                rc = 12;
                goto cleanup;
            }
	...
	if(NULL == (jclArg = strBufferCat(j9portLibrary, NULL, "-Xjcl:")))
		goto cleanup;
	if(NULL == (jclArg = strBufferCat(j9portLibrary, jclArg, J9_CLDC_DLL_NAME)))
		goto cleanup;


    /* Only allow the CDC library to be used with the j9cldc executable */
	if (!vmOptionsTableAddOption(&vmOptionsTable, strBufferData(jclArg), (void*) NULL))
		goto cleanup;
...
#if defined(JITNOCERT)
        case 11: /* case 11: -jit not certified on j9cldc executable and only allow -jcl:cldc */
            j9portLibrary->tty_printf(j9portLibrary, "VM startup error: JIT not available with j9cldc\n");
            break;
#endif
    case 12: /* case 12: cldc executable will only allow -jcl:cldc option */
        j9portLibrary->tty_printf(j9portLibrary, "VM startup error: j9cldc must use -jcl:cldc\n");
        break;
